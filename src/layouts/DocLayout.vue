...
const leftAside = computed(() => {
  ...
  const fm = frontmatter.value.aside
  if (fm == null) return theme.value.aside === 'left'
  return fm === 'left'
})

const outlineTitle = computed(() => {
  const cfg = theme.value.outline
  if (cfg && typeof cfg === 'object' && !Array.isArray(cfg) && (cfg as any).label) return (cfg as any).label
  ...
  <div class="min-h-screen flex flex-col bg-base-100 text-base-content" :class="pageClass">
    ...
    <div class="flex-1 w-full max-w-[88rem] mx-auto px-4 md:px-6 lg:px-8">
      <div class="flex gap-8">
        <aside v-if="hasSidebar" class="hidden lg:block w-64 shrink-0 py-8 sticky top-16 self-start max-h-[calc(100vh-4rem)] overflow-y-auto">
          <Sidebar />
        </aside>

        <aside
          v-if="hasAside && leftAside"
        ...
        >
          <aside class="absolute left-0 top-0 h-full w-72 bg-base-100 overflow-y-auto shadow-xl">
            <div class="flex items-center justify-between h-16 px-4 border-b border-base-300">
              <span class="font-semibold">{{ theme.sidebarMenuLabel ?? 'Menu' }}</span>
      ...
      <div v-if="tocOpen" class="xl:hidden fixed inset-0 z-40" @click.self="tocOpen = false">
        ...
        >
          <aside class="absolute right-0 top-0 h-full w-72 bg-base-100 overflow-y-auto shadow-xl">
            <div class="flex items-center justify-between h-16 px-4 border-b border-base-300">
              <span class="font-semibold">{{ outlineTitle }}</span>
